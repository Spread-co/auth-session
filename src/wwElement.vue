...
export default {
  ...
  setup(props) {
    ...
    const { value: wwIsMember, setValue: setWwIsMember } =
      wwLib.wwVariable.useComponentVariable({
        uid: 'isMember',
        name: 'Is Member',
        type: 'boolean',
    ...
    const { value: wwHouseholdId, setValue: setWwHouseholdId } =
      wwLib.wwVariable.useComponentVariable({
        uid: 'householdId',
        name: 'Household ID',
        type: 'string',
    ...
    const { value: wwAvatarUrl, setValue: setWwAvatarUrl } =
      wwLib.wwVariable.useComponentVariable({
        uid: 'avatarUrl',
        name: 'Avatar URL',
        type: 'string',
  ...
  methods: {
    ...
    async establishSession(session, supabaseUrl, supabaseAnonKey, { broadcast = true } = {}) {
      ...
      try {
        ...
        if (profiles?.[0]) {
          if (!displayName) displayName = profiles[0].display_name || '';
          avatarUrl = profiles[0].avatar_url || '';
        }
      } catch (_) {
        // Profile fetch is optional; displayName falls back to user metadata
      }

      // Check active membership
      try {
        const memberships = await client.from('user_memberships', {
          select: 'status',
          filters: `user_id=eq.${user.id}&status=in.(active,trialing,past_due)`,
          limit: 1,
        });
      ...
      // Get household ID from family_members
      try {
        const fm = await client.from('family_members', {
          select: 'family_id',
          filters: `user_id=eq.${user.id}`,
          limit: 1,
        });
        householdId = fm?.[0]?.family_id || '';
      } catch (_) {
...
.spread-auth-session__alert {
  ...
  gap: 0.5rem;
  padding: 10px 12px;
  background: var(--spread-error-bg);
  border: 1px solid var(--spread-error-border);
  border-radius: var(--spread-radius-md);
  font-size: 13px;
  color: var(--spread-error);
}
...
:global(html.dark) .spread-auth-session__input { background: #160c11; border-color: rgba(230,216,202,0.2); color: #e6d8ca; }
...
:global(html.dark) .spread-auth-session__heading { color: #e6d8ca; }
